...
import { renderPptxReport } from './reporting/renderers/pptxRenderer';
...
export async function generatePptxReport(
    ...
    const outputPath = path.join(REPORTS_DIR, `report-${scan.id}.pptx`);
    await renderPptxReport(report, outputPath);
    return outputPath;
}
